...
public partial class StadiumCMS_Create_Stadium : System.Web.UI.Page
{
    ...
    {
       // ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('', 'Stadium Infomation Has Been Modified Successfully');", true);
    ...
    private void FillSportCategory()
    {
        ...
        dt = dbFunctions.GetData("exec SP_GetSportCategory");
       ...
       dt = dbFunctions.GetData("select * from [MYA_Maleabna_Stadium] where StadiumID=" + Request.QueryString["id"]);
        ...
        if (dt.Rows.Count != 0)
        {
            ...
            if (!DBNull.Value.Equals(dt.Rows[0]["Gender"]))
                ...
                //DDLgender.Items.FindByText(dt.Rows[0]["Gender"].ToString().Trim()).Selected = true;


            DDLSportType.ClearSelection();
            if (!DBNull.Value.Equals(dt.Rows[0]["SportCategory"]))
                DDLSportType.SelectedValue = dt.Rows[0]["SportCategory"].ToString().Trim();
        ...
        #region CheckBoxCheckeOrNot
            ...
            if (dt.Rows.Count != 0)
            {
                Response.Write("StadiumType:"+dt.Rows[0]["StadiumType"].ToString());

                Response.Write("No of rows:"+dt.Rows.Count);
    ...
    public void lnkAdd_Click(object sender, EventArgs e)
    {
        ...
        if (lnkAdd.Text != "<i class='os-icon os-icon-ui-49'></i>&nbsp;Modify")
        {
            ...
            sqlCommand.CommandText = "insert into MYA_Maleabna_Stadium(GovernorateID,AreaID,SchoolID,StadiumName,StadiumNameEn,Address,AddressEn,KuwaitfinderNumber,KuwaitfinderLocation,GoogleMapLocation,Description,DescriptionEn,Note,StadiumMainImage,Noofplayers,Gender,SportCategory) values(@GovernorateID,@AreaID,@SchoolID,@StadiumName,@StadiumNameEn,@Address,@AddressEn,@KuwaitFinderNumber,@KuwaitFinderLocation,@GoogleMapLocation,@Description,@DescriptionEn,@Note,@StadiumMainImage,@Noofplayers,@Gender,@SportCategory)";
            ...
            sqlCommand.Parameters.AddWithValue("@SportCategory", DDLSportType.SelectedValue);
            ...
            try
            {
                ...
                if(DDLType.SelectedValue == "1")
                    addStadiumDetails(StrNewID, "Sevens", 5);
                else if (DDLType.SelectedValue == "2")
                {
                    ...
                    addStadiumDetails(StrNewID, "Elevens", 5);
                }
    ...
    protected void StadiumDetailsUpdate()
    {
        SqlConnection sqlConnection = new SqlConnection(dbFunctions.ConnectionString);
        ...
        sqlCommand.CommandText = "update MYA_Maleabna_Stadium_Detail set GovernorateID=@GovernorateID,AreaID=@AreaID,SchoolID=@SchoolID where StadiumID=@StadiumID";
    ...
    {
      //  Response.Write("InOutside");

        if (ViewState["StadiumType"] != null)
        {
            //Response.Write("InInside");
            if (ViewState["StadiumType"].ToString() == "0")
            {
                if (DDLType.SelectedValue == "1")
                    addStadiumDetails(Request.QueryString["id"].ToString(), "Sevens", 5);
                else if (DDLType.SelectedValue == "2")
                {
                    if (chkmembers.Checked)
                    {
                    ...
                    addStadiumDetails(Request.QueryString["id"].ToString(), "Elevens", 5);
                }
            }
            ...
            if (ViewState["StadiumType"].ToString() == "7")
            {
                ...
                {
                    UpdateStadiumDetails(Request.QueryString["id"].ToString(), "Sevens", "Elevens", 5);
                    if (chkmembers.Checked)
                    {
                        addStadiumDetails(Request.QueryString["id"].ToString(), "Sevens", 5);
                        addStadiumDetails(Request.QueryString["id"].ToString(), "Sevens", 5);
            ...
            else if (ViewState["StadiumType"].ToString() == "11")
            {
                if (DDLType.SelectedValue == "1")
                {
                    UpdateStadiumDetails(Request.QueryString["id"].ToString(), "Elevens", "Sevens", 5);
                }
                else if (DDLType.SelectedValue == "2")
                {
                    if (chkmembers.Checked)
