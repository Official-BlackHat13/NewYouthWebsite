...
public partial class View_BlockStadium : System.Web.UI.Page
{
    ...
    private void fillData()
    {
        ...
        if (lnkall.Visible == true)
        {
            str = str + ",CreatedBy=" + Session["MaleabnaCMSUserID"].ToString();
        }

        if (DDLGovernorate.SelectedValue != "0")
            str = str + ",b.GovernorateID=" + DDLGovernorate.SelectedValue + " ";

        if (DDLArea.SelectedValue != "0")
            str = str + ",b.AreaID=" + DDLArea.SelectedValue + " ";


        if (DDLSchool.SelectedValue != "0")
            str = str + ",b.SchoolID=" + DDLSchool.SelectedValue + " ";


        if (DDLStadium.SelectedValue != "0")
            str = str + ",b.StadiumID=" + DDLStadium.SelectedValue + " ";


        if (DDLStadiumCourt.SelectedValue != "0")
            str = str + ",b.StadiumCourtId=" + DDLStadiumCourt.SelectedValue + " ";

        if (ddlBlockBy.SelectedValue != "0")
            str = str + ",b.BlockBy='" + ddlBlockBy.SelectedItem.Text.Trim() + "' ";
        ...
        //cmd = "select b.TimeSlotBlockId,b.DateFrm,b.DateTo,CreatedBy,b.BlockBy,b.TimeSlotDetID,b.Status,g.GovernorateName,a.AreaName,s.SchoolName,st.StadiumName,sd.StadiumType from [MYA_Maleabna_TimeSlot_Block] b "
        //       +"join MYA_Maleabna_Governorate g on b.GovernorateID = g.GovernorateID "
        //       +"join MYA_Maleabna_Area a on b.AreaID = a.AreaID "
        //       +"join MYA_Maleabna_School s on b.SchoolID = s.SchoolID "
        //       +"join MYA_Maleabna_Stadium st on b.StadiumID = st.StadiumID "
        //       +"join MYA_Maleabna_Stadium_Detail sd  on b.StadiumCourtId = sd.StadiumDetId "
        //       + str + " order by TimeSlotBlockId desc";
