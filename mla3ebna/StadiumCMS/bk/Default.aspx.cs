...
public partial class StadiumCMS_Default : System.Web.UI.Page
{
    ...
    private void FillSiteStatistics()
    {
        ...
        cmd = "select count(Governorateid) as TotalGovernorate from MYA_Maleabna_Governorate";
        dt = dbFunctions.GetData(cmd);
        if (dt.Rows.Count != 0)
        {
            if (!DBNull.Value.Equals(dt.Rows[0]["TotalGovernorate"]))
                labTotalGovernorate.Text = dt.Rows[0]["TotalGovernorate"].ToString();

        }

        cmd = "select count(Areaid) as TotalArea from MYA_Maleabna_Area";
        dt = dbFunctions.GetData(cmd);
        if (dt.Rows.Count != 0)
        {
            if (!DBNull.Value.Equals(dt.Rows[0]["TotalArea"]))
                labTotalArea.Text = dt.Rows[0]["TotalArea"].ToString();

        }

        cmd = "select count(Schoolid) as TotalSchool from MYA_Maleabna_School";
        dt = dbFunctions.GetData(cmd);
        if (dt.Rows.Count != 0)
        {
            if (!DBNull.Value.Equals(dt.Rows[0]["TotalSchool"]))
                labTotalSchool.Text = dt.Rows[0]["TotalSchool"].ToString();

        }

        cmd = "select count(StadiumID) as TotalStadium from MYA_Maleabna_Stadium";
        dt = dbFunctions.GetData(cmd);
        if (dt.Rows.Count != 0)
        {
            if (!DBNull.Value.Equals(dt.Rows[0]["TotalStadium"]))
                labTotalStadium.Text = dt.Rows[0]["TotalStadium"].ToString();

        }

        cmd = "select count(GuardID) as TotalGuard from MYA_Maleabna_Guard";
        dt = dbFunctions.GetData(cmd);
        if (dt.Rows.Count != 0)
        {
            if (!DBNull.Value.Equals(dt.Rows[0]["TotalGuard"]))
                labTotalGuard.Text = dt.Rows[0]["TotalGuard"].ToString();

        }

        cmd = "select count(UserID) as TotalMembers from MYA_Maleabna_Members "; //where Status = '1'
        dt = dbFunctions.GetData(cmd);
        if (dt.Rows.Count != 0)
        {
            if (!DBNull.Value.Equals(dt.Rows[0]["TotalMembers"]))
                labTotalMembers.Text = dt.Rows[0]["TotalMembers"].ToString();

        }

        cmd = "select count(BookingID) as TotalBooking from MYA_Maleabna_Booking where BookingStatus ='1'";
        dt = dbFunctions.GetData(cmd);
        if (dt.Rows.Count != 0)
        {
            if (!DBNull.Value.Equals(dt.Rows[0]["TotalBooking"]))
                labTotalBooking.Text = dt.Rows[0]["TotalBooking"].ToString();

        }


        cmd = "select count(BookingID) as TotalBookingCancel from MYA_Maleabna_Booking where BookingStatus ='2'";
        dt = dbFunctions.GetData(cmd);
        if (dt.Rows.Count != 0)
        {
            if (!DBNull.Value.Equals(dt.Rows[0]["TotalBookingCancel"]))
    ...
    private void FillLatestBooking()
    {
        ...
        cmd = "select top 8 *  from V_Booking where BookingStatus='1' order by BookingID desc";
