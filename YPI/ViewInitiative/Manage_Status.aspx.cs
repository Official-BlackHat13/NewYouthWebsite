...
public partial class ViewInitiative_Manage_Status : System.Web.UI.Page
{
    int i;
    string str;
    string[] arr;
    ...
    private void fillData()
    {
        ...
        cmd = "select * from [MYA_PI_Status]  order by [sort] asc ";
        ...
        for (i = 0; i <= dg.Items.Count; i++)
        {
            if (i > 0)
            {
                ...
                if (chkBox2.Checked == true)
                {
                    ...
                    sqlstr = "update [MYA_PI_Status] set [status]=1 where id=" + pId;
                    dbFunctions_YPI.ExecuteQuery(sqlstr);
                }
                if (chkBox2.Checked == false)
                {
                    pId = Convert.ToInt32(dgItem.Cells[1].Text);
                    sqlstr = "update [MYA_PI_Status] set [status]=0 where id=" + pId;
                    dbFunctions_YPI.ExecuteQuery(sqlstr);
                }
            }
        }
    ...
    protected void lk_del_Click(System.Object sender, System.EventArgs e)
    {
        ...
        for (i = 0; i <= dg.Items.Count - 1; i++)
        {
            ...
            {
                cmd = "delete from [MYA_PI_Status] where [ID] = " + dg.Items[i].Cells[1].Text;
                dbFunctions_YPI.ExecuteQuery(cmd);
                ViewInitiativeAppUserActivityLog.CreateUserActivityLog(ViewInitiativeAppCurrentUser.MYAPIVIAppUserID, ViewInitiativeAppCurrentUser.MYAPIVIAppName, "Status", "Delete", DateTime.Now, "" + dg.Items[i].Cells[1].Text + "", "" + dg.Items[i].Cells[2].Text + "", "");
            }
        }
        fillData();
    }
    ...
    public void dg_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "Update")
        {
            ...
            sqlCommand.CommandText = "update MYA_PI_Status set NameAr=@NameAr where id=@id";
            ...
            try
            {
                ...
                ViewInitiativeAppUserActivityLog.CreateUserActivityLog(ViewInitiativeAppCurrentUser.MYAPIVIAppUserID, ViewInitiativeAppCurrentUser.MYAPIVIAppName, "Status", "Edit", DateTime.Now, "" + e.Item.Cells[1].Text + "", "" + globalFunctions.checkText(((TextBox)e.Item.Cells[2].Controls[0]).Text) + "", "");
                ClientScript.RegisterStartupScript(this.GetType(), "SweetAlert", "swal('', 'Status Infomation Has Been Updated Successfully', 'success');", true);
                dg.EditItemIndex = -1;
                fillData();
            }
            catch (Exception ex)
    ...
    public void lnkSort_Click(object sender, EventArgs e)
    {
        ...
        for (i = 0; i <= dg.Items.Count - 1; i++)
        {
            ...
            sqlCommand.CommandText = "update MYA_PI_Status set sort=@sort where id=@id";
        ...
        ClientScript.RegisterStartupScript(this.GetType(), "SweetAlert", "swal('', 'Status Sort Has Been Updated Successfully', 'success');", true);
