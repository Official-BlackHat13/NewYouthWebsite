    ...
    private void LoadLevel()
    {
        ...
        if (!HdAge.Value.Equals(""))
        {
            ...
            if (ageval >= 7 && ageval <= 10)
            {
                ...
                DDLCatagory.Items[6].Attributes.Add("class", "hiddenR");
            ...
            else if (ageval >= 11 && ageval <= 14)
            {
                ...
                DDLCatagory.Items[5].Attributes.Add("class", "");
                DDLCatagory.Items[6].Attributes.Add("class", "");
                ScriptManager.RegisterStartupScript(Page, this.GetType(), "visible", " $get('" + btnSubmit.ClientID + "').style.display = 'inline';", true);
            }
            ...
            if (Page.IsValid)
            {
                ...
                for (int i = 0; i < hfc.Count; i++)
                {
                    ...
                    if (hpf.ContentLength > 0)
                    {
                        ...
                        Command.Parameters.AddWithValue("@Organization", SqlDbType.Int).Value = int.Parse(Session["OrganizationID"].ToString());
