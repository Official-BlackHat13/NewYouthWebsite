...
public partial class youth_NEW_ADMIN_index : System.Web.UI.Page
{
    ...
    protected void btlogin_Click(object sender, EventArgs e)
    {
        try
        {
            if (!txtusername.Text.Equals("") & (!txtpassword.Text.Equals("")))
            {
                ...
                cmd1 = new SqlCommand("YCLC_EvalLogin", con);
                ...
                cmd1.Parameters.AddWithValue("@type", SqlDbType.NVarChar).Value = "login"; 
                ...
                if (ds.Rows.Count > 0)
                {
                    col1Value = ds.Rows[0]["Name"].ToString().Trim();
                    string type = ds.Rows[0]["OrganizationID"].ToString();
                    id = ds.Rows[0]["id"].ToString();
                    Session["userid_Yclid"] = id;
                    Session["name"] = col1Value;
                    Session["OrganizationID"] = type;
                   Session["Type"] = ds.Rows[0]["Type"].ToString(); 
    ...
    protected void bt_Click(object sender, EventArgs e)
    {
        if (txtEmail.Text.Trim() != "")
        {
            ...
            SQLConnection();
            SqlCommand cmd1 = new SqlCommand("YCLC_EvalLogin", con);
            cmd1.Parameters.AddWithValue("@email", SqlDbType.NVarChar).Value = txtEmail.Text;
            cmd1.Parameters.AddWithValue("@type", SqlDbType.NVarChar).Value = "forgot";
            cmd1.CommandType = CommandType.StoredProcedure;
            SqlDataAdapter da = new SqlDataAdapter(cmd1);
            DataTable ds = new DataTable();
            da.Fill(ds);
            if (ds.Rows.Count > 0)
            {
                string pwd = ds.Rows[0]["Password"].ToString();
                MailMessage message1 = new MailMessage();
                message1.From = new MailAddress("web@youth.gov.kw", "Forgot Password for The System");
                message1.To.Add(new MailAddress(ds.Rows[0]["Email"].ToString().Trim()));
                message1.Subject = "no-replay";
                message1.Body = "Your Current Password is" + " " + pwd;
